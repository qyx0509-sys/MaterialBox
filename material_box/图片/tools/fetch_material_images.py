...
def write_generated_image_map(path: Path, image_map: dict[str, dict[str, Any]]) -> None:
    ...
    content = f"""// Auto-generated by tools/fetch_material_images.py.
// Merge each image type independently so a new macro image cannot remove an
// existing manual micro or structure image.
(function () {{
  const incoming = {payload};
  const merge = window.MaterialBoxMergeImageMaps || function (base, generated) {{
    const output = {{ ...(base || {{}}) }};
    Object.entries(generated || {{}}).forEach(([materialId, groups]) => {{
      const current = output[materialId] || {{}};
      output[materialId] = {{ ...current }};
      ["macro", "micro", "structure"].forEach((type) => {{
        const seen = new Set();
        output[materialId][type] = [...(current[type] || []), ...(groups[type] || [])]
          .filter((item) => item && item.src && !seen.has(item.src) && seen.add(item.src));
      }});
    }});
    return output;
  }};
  window.MATERIAL_IMAGE_MAP = merge(window.MATERIAL_IMAGE_MAP || {{}}, incoming);
}})();
"""
